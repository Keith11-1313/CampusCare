...
function toggleStatus(id){showConfirm('Toggle Status?','Change status?','Yes').then(r=>{if(r.isConfirmed){const fd=new FormData();fd.append('action','toggle_status');fd.append('id',id);fd.append('csrf_token','<?php echo getCSRFToken(); ?>');fetch('year_levels.php',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{showToast(d.success?'success':'error',d.message);if(d.success)setTimeout(()=>location.reload(),800);});}});}
document.getElementById('ylForm').addEventListener('submit',function(e){e.preventDefault();fetch('year_levels.php',{method:'POST',body:new FormData(this)}).then(r=>r.json()).then(d=>{if(d.success){ylModal.hide();showToast('success',d.message);setTimeout(()=>location.reload(),800);}else showToast('error',d.message);});});
</script>
